...
const t: Record<Lang, any> = {
  en: {
    ...
    settingsSub: 'Manage your profile, account preferences and platform language',
    profile: 'Profile',
    account: 'Account',
  ...
  id: {
    ...
    settingsSub: 'Kelola profil, preferensi akun, dan bahasa platform Anda',
    profile: 'Profil',
    account: 'Akun',
    appearance: 'Tampilan',
...
export default function SettingsPage() {
  ...
  const handleProfileSave = async (e: React.FormEvent) => {
    ...
    setError('')
    setSuccess('')
    console.log('[Settings] Saving profile:', { userId: currentUser?.id, payload: { name: form.name, email: form.email, phone: form.phone } })
    try {
  ...
  const handlePasswordSave = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    console.log('[Settings] Saving password for user:', currentUser?.id)
    try {
      const payload: any = { name: form.name, email: form.email, phone: form.phone }
      if (form.newPassword) {
      ...
      setSuccess(txt.saved)
      console.log('[Settings] Password saved successfully')
    } catch (err: any) {
  ...
  const handleDeleteAccount = async () => {
    ...
    setLoading(true)
    console.log('[Settings] Deleting account:', currentUser?.id)
    try {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError('')
    setSuccess('')
    console.log('[Settings] Uploading avatar:', { userId: currentUser.id, fileName: file.name, size: file.size })
    try {
      const res = await api.uploadAvatar(currentUser.id, file)
      setAvatarUrl(res.avatar)
      setStoredUser({ ...currentUser, avatar: res.avatar })
      setSuccess(txt.saved)
      console.log('[Settings] Avatar uploaded:', res.avatar)
    } catch (err: any) {
                ...
                <div className="rounded-2xl p-6" style={{ background: CARD, border: `1px solid ${BORDER}`, boxShadow: shadow }}>
                  <div className="flex items-center gap-5 mb-6">
                    ...
                    <div>
                      ...
                      <span className="inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold" style={{ background: AL, color: A }}>{txt.admin}</span>
                    </div>
                  ...
                  <form onSubmit={handleProfileSave} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input value={txt.admin} disabled
